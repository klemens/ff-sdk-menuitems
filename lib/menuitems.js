...
function updateMenuitemAttributes(menuitem, options) {
  ...
  if (options.image) {
    updateImage(menuitem, options.image);
  }
  ...
  if (options.observes)
    menuitem.setAttribute("observes", options.observes);
...
function updateImage(menuitem, val) {
  if(val) {
    menuitem.classList.toggle("menuitem-iconic", true);
    menuitem.style.listStyleImage = "url('" + val + "')";
  } else {
    menuitem.classList.toggle("menuitem-iconic", false);
    menuitem.style.removeProperty("list-style-image");
  }
}
